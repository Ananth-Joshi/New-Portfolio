  ...
  return (
    <div className="max-w-4xl w-full">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-serif text-white">Manage Experience</h1>
        <Link 
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-zinc-800">
          {experiences?.map((exp) => (
            <div key={exp.id} className="p-4 space-y-3">
              <div className="min-w-0">
                <p className="font-medium text-white break-words">{exp.company}</p>
                <p className="text-sm text-zinc-400 break-words">{exp.role}</p>
                <p className="text-xs text-zinc-500 mt-0.5">Order: {exp.display_order}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <span className={`px-2 py-0.5 text-xs font-medium rounded-full border ${exp.show_on_home ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20'}`}>
                  {exp.show_on_home ? 'On home' : 'Not on home'}
                </span>
              </div>
              <div className="flex gap-2 pt-1">
                <Link href={`/admin/experience/${exp.id}`} className="flex-1 text-center px-3 py-2 text-sm text-blue-400 hover:text-blue-300 border border-zinc-700 rounded-md hover:bg-zinc-800/60 transition-colors">Edit</Link>
                <form action={deleteExperience} className="flex-1">
          ...
          {!experiences?.length && (
            <p className="px-6 py-8 text-center text-zinc-500">No experiences found.</p>
          )}
        </div>
